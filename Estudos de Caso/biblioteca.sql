...
select livro.titulo,livro.ano_edicao,editora.nome,
editora.estado
from livro inner join editora
on livro.id_editora = editora.id_editora;

select autor.nome,autor.nacionalidade,livro.titulo,
livro.ano_Edicao
from autor inner join livro
on autor.id_autor = livro.id_autor;

select aluno.matricula, aluno.nome, aluno.curso, 
...
select autor.nome,autor.nacionalidade,livro.titulo,
livro.ano_edicao,editora.nome,editora.cidade
from autor inner join livro
on autor.id_autor = livro.id_autor
inner join editora
on livro.id_editora = editora.id_editora;

select aluno.matricula,aluno.nome,aluno.curso,emprestimo.data_emprestimo,
emprestimo.data_devolucao,livro.titulo,livro.ano_edicao
from aluno inner join emprestimo
on aluno.id_aluno = emprestimo.id_aluno
inner join livro
...
select aluno.matricula,aluno.nome,aluno.curso,emprestimo.data_emprestimo,
emprestimo.data_devolucao,datediff(emprestimo.data_devolucao, emprestimo.data_emprestimo) as dias,
livro.titulo,livro.ano_edicao,editora.nome as nome_editora
from aluno inner join emprestimo
on aluno.id_aluno = emprestimo.id_aluno
inner join livro
on emprestimo.id_livro = livro.id_livro
...
-- Left é a tabela anterior e o Right é a Proxima Tabela.


-- inner join: Busca as relações entre as duas tables. (o que elas tem em comum):
select livro.titulo,livro.ano_edicao,editora.nome,editora.estado
from livro inner join editora
on livro.id_editora = editora.id_editora;

-- right join: Busca as relações entre as duas tables. (o que elas tem em comum):
select livro.titulo,livro.ano_edicao,editora.nome,editora.estado
from livro right join editora
on livro.id_editora = editora.id_editora
where livro.titulo is null;

-- left join: Busca as relações entre as duas tables. (o que elas tem em comum):
select livro.titulo,livro.ano_edicao,editora.nome,editora.estado
...
-- Quais editoras não tem nenhum livro vinvulado???:
select livro.titulo,livro.ano_edicao,editora.nome,editora.estado
from livro right join editora
on livro.id_editora = editora.id_editora
where livro.titulo is null;

select aluno.nome, emprestimo.id_emprestimo
from aluno left join emprestimo
on aluno.id_aluno = emprestimo.id_aluno
where emprestimo.id_aluno is null;

select count(id_livro),ano_edicao
from livro 
group by ano_edicao;
...
select count(id), nacionalidade,idade
from atletas
where idade > 30
group by nacionalidade;
