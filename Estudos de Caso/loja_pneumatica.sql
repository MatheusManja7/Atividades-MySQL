create database loja_pneumatica;
use loja_pneumatica;
...
-- 01
select total_vendas,unidade from vendas;

-- 02
select total_vendas,mes from vendas;

-- 03
select sum(total_vendas) as total_vendas
from vendas;

-- 04
select mes, sum(vendas.total_vendas)
from vendas
where mes = "Marzo"
group by mes;
...
-- 06
select vendedor.nome, vendas.total_vendas
from vendedor left join vendas
on vendedor.id_vendedor = vendas.id_vendedor
where vendas.total_vendas is null
group by vendedor.nome, vendas.total_vendas;

-- 07 
select vendedor.id_vendedor, vendedor.nome,
sum(vendas.total_vendas), sum(vendas.total_vendas * vendedor.comissao) as salario_vendedor
from vendedor inner join vendas
on vendedor.id_vendedor = vendas.id_vendedor
group by vendedor.id_vendedor, vendedor.nome
order by salario_vendedor desc limit 5;

-- 08
select vendedor.id_vendedor, vendedor.nome, sum(vendas.total_vendas)
from vendedor left join vendas
on vendedor.id_vendedor = vendas.id_vendedor
group by vendedor.id_vendedor, vendedor.nome
having sum(vendas.total_vendas) > 350000;
